...
/* Weak diagnostic hook: lets a keyboard observe the raw USB events (RESET,
 * ADDRESS, CONFIGURED, SUSPEND, ...) as they arrive, without touching the
 * event queue. The default is a no-op. */
__attribute__((weak)) void usb_event_debug(usbevent_t event) {
    (void)event;
}

/* Handles the USB driver global events. */
static void usb_event_cb(USBDriver *usbp, usbevent_t event) {
    usb_event_debug(event);
